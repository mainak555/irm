...
`/admin/auth/callback.php` SHALL be publicly accessible without an authentication guard. It SHALL render an informational page containing the text "SSO callback not yet implemented" and a link to `/admin/login.php`. It SHALL NOT write to `$_SESSION['auth']`, SHALL NOT exchange any tokens, and SHALL NOT act on any query parameters received from the provider.

#### Scenario: Callback page accessible without a session
- **WHEN** an unauthenticated visitor navigates to `/admin/auth/callback.php`
- **THEN** the page renders without redirecting to the login page

#### Scenario: Callback page displays not-implemented notice
- **WHEN** any visitor navigates to `/admin/auth/callback.php`
- **THEN** the response contains the text "SSO callback not yet implemented"
- **THEN** the response contains a link to `/admin/login.php`

#### Scenario: Callback does not create an authenticated session
- **WHEN** a visitor navigates to `/admin/auth/callback.php` with `code` and `state` query parameters present
- **THEN** `$_SESSION['auth']` is NOT set
- **THEN** the page renders the not-implemented notice regardless of the query parameters
