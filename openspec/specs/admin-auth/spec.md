...
When the `auth_users` table has zero rows, the system SHALL display a setup screen instead of the login form. The setup screen SHALL collect only a password for the built-in admin account. On submission, the system SHALL create one `auth_users` row with `username='admin'`, `name='Administrator'`, `role='sa'`, and the bcrypt-hashed password. Only one `sa` account SHALL ever exist; the setup screen SHALL NOT appear once any row exists in `auth_users`.
...
- **THEN** one `auth_users` row is inserted with `username='admin'`, `name='Administrator'`, `role='sa'`, and the bcrypt hash stored in `password`
...
The login form SHALL accept `username` and `password` fields. The system SHALL look up the user by `username`, verify the submitted password against the stored bcrypt hash using `password_verify()`, and check that `is_active = 1`. On success, the system SHALL write `$_SESSION['auth']` containing `id`, `name`, `role`, and `theme`, then redirect to `/admin/index.php`.

#### Scenario: SA logs in with correct credentials
- **WHEN** the sa user submits `username='admin'` and the correct password
...
#### Scenario: Login rejected for non-existent username
- **WHEN** a login form is submitted with a username that has no matching row
- **THEN** the login page is re-displayed with a generic authentication error
