...
`config/menu.json` is the runtime nav source for all public pages (no DB query on page render) and the seed source for the `menus` DB table on fresh installations. Admin panel changes to navigation must write back to `menu.json` to take effect publicly.
