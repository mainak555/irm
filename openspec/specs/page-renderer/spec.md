...
`includes/page_renderer.php` provides the `render_page_layout()` function that walks a page layout array and outputs Bootstrap-grid HTML. It handles three column types — `html`, `embed`, and `component` — and is the sole rendering engine for public designer pages.
...
`includes/page_renderer.php` SHALL also expose `render_page_layout(array $rows): void` for backward compatibility with pages that use the rows-based schema. It walks the `$rows` array and outputs Bootstrap-grid HTML. It SHALL require no global state and SHALL be safe to call from any PHP file that has already included `includes/config.php`.
